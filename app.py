...
def pdf_rapor():
    ...
    if cid:
        try:
            ...
            if img_resp.status_code == 200:
                ...
                story.append(Paragraph("Molekul Yapisi", ParagraphStyle('h2', fontName=FONT_BOLD, fontSize=13, spaceAfter=6)))
    ...
    if piktogramlar:
        story.append(Paragraph("GHS Piktogramlari", ParagraphStyle('h2', fontName=FONT_BOLD, fontSize=13, spaceAfter=6)))
        pikt_text = "  |  ".join([p['isim'] for p in piktogramlar])
        story.append(Paragraph(pikt_text, ParagraphStyle('normal', fontName=FONT, fontSize=10)))
        story.append(Spacer(1, 0.5*cm))

    if h_kodlari:
        story.append(Paragraph("Tehlike Bildirimleri (H Kodlari)", ParagraphStyle('h2', fontName=FONT_BOLD, fontSize=13, spaceAfter=6)))
        story.append(Spacer(1, 0.2*cm))

        h_data = [['Kod', 'Aciklama', 'Kategori']]
        for h in h_kodlari:
    ...
    story.append(Paragraph(f"Kaynak: PubChem | kimyasal.onrender.com",
                            ParagraphStyle('footer', fontName=FONT, fontSize=8, textColor=colors.grey)))
